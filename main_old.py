...
# def measure_curve_rad(old_coefficients, y0):
    ym_per_pix = 3.48 / 93  # meters per pixel in y dimension
    xm_per_pix = 3.66 / 748  # meters per pixel in x dimension

    coefficients =np.array([ym_per_pix/(xm_per_pix**2), ym_per_pix/xm_per_pix, ym_per_pix])*old_coefficients
    curve_rad = (
                    (1 + (
                        2 * coefficients[0] * y0 * ym_per_pix + coefficients[1]) ** 2) ** 1.5) / (
                    2 * coefficients[0])

    x0 = coefficients[0] * (y0 ** 2) + coefficients[1] * y0 + coefficients[2]
...
class Lanes:
    ...
    def fit(self, thresholded, original=None, M=None, vidcap=None):
        """
        ...
        for side in Lanes.Side:
            ...
            if len(point_coords[0]) > 0:
                ...
                if self.smoothing_coefficients[side] is None:
                    ...
                    new_smoothing_coefficients[side] = new_coefficients[side]
                # Measure and store the curvature radius and center of curvature
                new_x0[side], new_y0[side], new_curve_rad[side] = measure_curve_rad(point_coords[0], point_coords[1])
        ...
        if original is not None:
            overlay_img = self.overlay_top_view(original, self.counter)
            # Fetch an image with marked lanes on overlay
            if M is not None:
                overlay_img = self.overlay_lanes(overlay_img, M)
            if self.plot is None:
                plt.ion()
                self.plot = plt.imshow(switch_RGB(overlay_img))
            else:
                self.plot.set_data(switch_RGB(overlay_img))
            plt.draw()
            plt.pause(0.001)

    def overlay_top_view(self, image, counter = None):
        """
        ...
        sanity = ((self.x0[0] - self.x0[1]) ** 2 + (
        self.y0[0] - self.y0[1]) ** 2) ** .5 if None not in self.x0 and None not in self.y0 else 0
        ...
        if counter is not None:
            to_print = 'Frame# '+str(counter)+' '+to_print
...
def main():
    ...
    while (True):
        ...
        counter +=1
        overlay_img = lanes.overlay_top_view(undistorted_img, counter)
        # Fetch an image with marked lanes on overlay
